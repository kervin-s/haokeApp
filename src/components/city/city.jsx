      ...
      <div className="city_group" key={key} style={style}>
        ...
        <ul>
          {this.state.oCityList[item].map((val) => (
            <li key={val.value} onClick={() => this.fnSetCity(val.label)}>
              {val.label}
            </li>
          ))}
        </ul>
      ...
      <div className={this.props.sClass}>
        <div className="city_title">
          <span
            ...
            onClick={() => this.props.fnSwitch("city_wrap")}
          ></span>
          <h3>选择城市</h3>
        </div>
        ...
        <ul className="city_index">
          {aCityKeys.map((item, i) => (
            <li
              key={item}
              className={iNow === i ? "active" : ""}
              onClick={() => this.fnScrollToRow(i)}
            >
